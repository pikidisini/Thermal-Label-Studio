...
export default function RightInspector({
  ...
  const getObjectIcon = (obj) => {
    ...
    if (obj.dataQr) return <QrCode className="w-3.5 h-3.5 text-indigo-400" />;
  ...
  const getObjectLabel = (obj) => {
    ...
    if (obj.dataQr) return '2D Matrix / QR';
                ...
                objectsList.map((item, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`px-sm py-1.5 flex items-center justify-between cursor-pointer group transition ${
                        isSelected
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="font-label-sm text-label-sm truncate max-w-[130px]">
                          {getObjectLabel(item)}
                        </span>
                      </div>

                      <div className="shrink-0">{getObjectBadge(item)}</div>
                    </div>
                  );
                })
              )}
      ...
      <div className="border-t border-outline-variant bg-surface-container-lowest p-sm shrink-0">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="font-code-matrix text-code-matrix text-primary bg-surface-container-high px-1 py-0.5 border border-outline-variant">
            {obj ? (obj.isBarcode ? 'Barcode 1D' : obj.type?.toUpperCase()) : 'NO SELECTION'}
          </span>
        </div>
