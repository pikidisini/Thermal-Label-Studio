...
function StudioCanvas({
  ...
  useEffect(() => {
    ...
    fabricCanvas.on('object:moving', (e) => {
      ...
      const isSnap = callbacksRef.current.isSnapEnabled !== false;
      const isGuides = callbacksRef.current.areGuidesEnabled !== false;

      if (isSnap) {
        const snapStep = (callbacksRef.current.gridSizeMm || 2.5) * pxPerMm; // default 10px
        const snappedLeft = Math.round(obj.left / snapStep) * snapStep;
        const snappedTop = Math.round(obj.top / snapStep) * snapStep;
    ...
    // Precise Canvas Coordinate Extractor (immune to CSS translate3d, zoom, and viewport offsets)
    const getCanvasPointer = (e) => {
      ...
      if (canvasRef.current) {
        return canvasRef.current.getPointer(e);
      }
      return { x: 0, y: 0 };
    };

    // Interactive CAD Drawing Listeners (Illustrator / Inkscape continuous creation workflow)
    ...
    fabricCanvas.on('mouse:up', (opt) => {
      ...
      const currentTool = activeToolRef.current;
      const pointer = opt?.e ? getCanvasPointer(opt.e) : drawStartPosRef.current;
      const origX = drawStartPosRef.current.x;
      const origY = drawStartPosRef.current.y;
      ...
      const finalizeObject = (obj) => {
        ...
        callbacksRef.current.onCanvasModified?.();
        // Tool stays active so user can continuously draw multiple objects of the same type!
      };
        ...
        if (dataUrl) {
          fabric.Image.fromURL(dataUrl, (img) => {
            ...
            img.set({
              ...
              dataBarcode: tokenOrValue.replace(/[{}]/g, '').trim(),
            });
            finalizeObject(img);
          });
        }
          ...
          if (dataUrl) {
            fabric.Image.fromURL(dataUrl, (img) => {
              ...
              img.set({
                ...
                barcodeType: 'qrcode',
                barcodeValue: tokenOrValue,
                dataQr: tokenOrValue.replace(/[{}]/g, '').trim(),
              });
              finalizeObject(img);
            });
          }
    ...
    const handleKeyDown = (e) => {
      // Check if user is currently typing in an HTML input, textarea, or contentEditable
      const activeEl = document.activeElement;
      const isHtmlInput = activeEl && (
      ...
      // Select All on Canvas Shortcut (Ctrl+A / Cmd+A)
      if ((e.ctrlKey || e.metaKey) && (e.key === 'a' || e.key === 'A')) {
        e.preventDefault();
        const selectableObjs = fabricCanvas.getObjects().filter((o) => o.selectable !== false && o.visible !== false);
        if (selectableObjs.length > 0) {
          fabricCanvas.discardActiveObject();
          const activeSel = new fabric.ActiveSelection(selectableObjs, {
            canvas: fabricCanvas,
          });
          fabricCanvas.setActiveObject(activeSel);
          fabricCanvas.requestRenderAll();
          callbacksRef.current.onSelectionChanged?.(activeSel);
        }
        return;
      }
  ...
  // Track dimension changes to reset panOffset to center
  const prevDimensionsRef = useRef({ w: canvasWidthPx, h: canvasHeightPx });
  useEffect(() => {
    if (
      prevDimensionsRef.current.w !== canvasWidthPx ||
      prevDimensionsRef.current.h !== canvasHeightPx
    ) {
      prevDimensionsRef.current = { w: canvasWidthPx, h: canvasHeightPx };
      panOffsetRef.current = { x: 0, y: 0 };
      setPanOffset({ x: 0, y: 0 });
    }
  }, [canvasWidthPx, canvasHeightPx]);

  // Update canvas dimensions and zoom dynamically without disposing the canvas
  useEffect(() => {
    ...
    drawRulers();
  }, [canvasWidthPx, canvasHeightPx, zoom, drawRulers]);
    ...
    >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <canvas
          ...
          width={24}
          style={{ backgroundColor: '#1a1b1e' }}
          className="w-6 h-full block border-r border-studio-border pointer-events-none z-20 shrink-0"
        />
        ...
        <div
          ...
          style={{ touchAction: 'none', backgroundColor: '#121316' }}
        >
          ...
          <div
            ...
            className="canvas-container absolute shadow-2xl overflow-hidden border border-gray-400/30 shrink-0 will-change-transform"
          ...
          >
            <canvas ref={canvasElRef} data-testid="fabric-canvas" style={{ backgroundColor: '#ffffff' }} />
          </div>
        </div>
      </div>
    </div>
